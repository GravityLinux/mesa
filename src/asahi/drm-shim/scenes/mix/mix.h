...
run_mix(unsigned width, unsigned height)
{
   ...
   if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
      fail("mix framebuffer");
   glViewport(0, 0, width, height);
   glDisable(GL_CULL_FACE);
   glEnable(GL_DEPTH_TEST);
   glDepthFunc(GL_ALWAYS);
   glClearDepthf(1);
   glUniform4f(glGetUniformLocation(program, "first"), .125, .625, .25, .75);
   glUniform4f(glGetUniformLocation(program, "second"), .875, .125, .75, .25);
   ...
   for (unsigned frame = 0; frame < frames; ++frame) {
      ...
      if (glGetError() != GL_NO_ERROR)
         fail("mix draw");
      printf("T8132_MIX_FRAME frame=%u\n", frame);
   }
