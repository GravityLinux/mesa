...
run_control_flow(unsigned width, unsigned height)
{
   ...
   if (!linked)
      fail("control-flow program link");
   glDeleteShader(v);
   glDeleteShader(f);
   ...
   for (unsigned frame = 0; frame < frames; ++frame) {
      ...
      if (glGetError() != GL_NO_ERROR)
         fail("control-flow draw");
      printf("T8132_CONTROL_FLOW_FRAME frame=%u\n", frame);
   }
