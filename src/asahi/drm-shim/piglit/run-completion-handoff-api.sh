...
${CC:-cc} -std=c11 -Wall -Wextra -Werror \
    "$script_dir/tests-graphics/completion-handoff/api.c" -o "$results/completion-handoff-api" \
    $(pkg-config --cflags --libs epoxy egl) -lm
export ASAHI_GRAPHICS_BINARY="$results/completion-handoff-api"
export ASAHI_GRAPHICS_EXPECTED_RESULTS=5
